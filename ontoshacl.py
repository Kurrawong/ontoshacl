...
class Shacl:
    def __init__(
        ...
        self.graph.add(
            (
                ...
                OWL.versionInfo,
                Literal(
                    f"{versionIRI.n3(namespace_manager=self.graph.namespace_manager)}: Generated by OntoShacl:v{SCRIPT_VERSION}"
                ),
            )
        )
    ...
    def add_nodeshape(
        ...
        for property_shape in self.property_shapes(for_nodeshape=shape_uri):
            ...
            klass_restrictions = list(self.graph.objects(property_shape, SH["class"]))
            min = self.graph.value(property_shape, SH.minCount, None)
            max = self.graph.value(property_shape, SH.maxCount, None)
            if not any([min, max, klass_restrictions]):
                return
            message = ""
            if all([min, max]):
                if min == max:
                    message += f"A {klass_str} must be the target of exactly {min} {path_str} statements"
                else:
                    message += f"A {klass_str} must have between {min} and {max} {path_str} statements"
            elif min:
                message += (
                    f"A {klass_str} must have at least {min} {path_str} statements"
                )
            elif max:
                message += f"A {klass_str} must not have more than {max - 1} {path_str} statements"
            if klass_restrictions:
                if any([min, max]):
                    message += "\n and "
                message += (
                    f"The object of the {path_str} property on a {klass_str} must be "
                )
                if len(klass_restrictions) > 1:
                    message += "one of ["
                    for klass_restriction in klass_restrictions:
                        message += f"{klass_restriction.n3(namespace_manager=self.graph.namespace_manager)},"
                    message += "]"
                else:
                    message += f"a {klass_restrictions[0].n3(namespace_manager=self.graph.namespace_manager)}"
